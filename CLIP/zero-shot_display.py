...
log_file = open('prompt_ensemble_test_log.txt','w+')
#res_file = open('prompt_ensemble_test_res.txt','w+')
...
train_path = "/home/ai4sg/meiliu/CLIP/CLIP/landuse/zero-shot_vis/" #"/home/ai4sg/meiliu/CLIP/CLIP/landuse/NewYork_test/"
...
def get_features(dataset, lu_list):
	...
	with torch.no_grad():
		...
		text_tokens1 = clip.tokenize([f"{c} purpose" for c in lu_list]).cuda()
		text_features1 = model.encode_text(text_tokens1).float()

		text_tokens2 = clip.tokenize([f"{c} use" for c in lu_list]).cuda()
		text_features2 = model.encode_text(text_tokens2).float()

		text_features = text_features1 * 0.881 + text_features2 * 0.119
...
top_probs, top_labels = similarity.cpu().topk(6, dim=-1)

# save the results
plt.figure(figsize=(16, 8))

for i, filename in enumerate(train_list):
	image = Image.open(os.path.join(train_path, filename)).convert("RGB")
	...
	plt.yticks(y, [lu_list[index] for index in top_labels[i].numpy()])
	plt.xlabel("prediction probability")

#plt.subplots_adjust(wspace=0.5)
plt.subplots_adjust(wspace=0.5, hspace=0.5, left=0.1, bottom=0.1, right=0.96, top=0.96)
#plt.show()

...
log_file = open('log_file_zeroshot_ViT-L14_purpose-Brooklyn.txt','w+')
res_file = open('res_file_zeroshot_ViT-L14_purpose-Brooklyn.txt','w+')
...
# train_path = "/home/bpeng/meiliu/landuse/NewYork_nonmixed_100m/"
train_path = "/home/bpeng/meiliu/landuse/Brooklyn/"
# train_path = "/home/bpeng/meiliu/landuse/Manhattan/"
# train_path = "/home/bpeng/meiliu/landuse/Queens/"
# train_path = "/home/bpeng/meiliu/landuse/Bronx/"
...
def get_features(dataset, lu_list):
	...
	with torch.no_grad():
		...
		# use
		# text_tokens1 = clip.tokenize([c + " purpose in New York" for c in lu_list]).cuda()
		# text_features1 = model.encode_text(text_tokens1).float()
		# # purpose
		# text_tokens2 = clip.tokenize([c + " purpose" for c in lu_list]).cuda()
		# text_features2 = model.encode_text(text_tokens2).float()
		# # no template
		# text_tokens3 = clip.tokenize([c for c in lu_list]).cuda()
		# text_features3 = model.encode_text(text_tokens3).float()

		# text_features = text_features1 * 0.86 + text_features2 * 0.05 + text_features3 * 0.09

		# spatial awareness prompt engineering
		text_tokens = clip.tokenize([c + " purpose in Brooklyn" for c in lu_list]).cuda()
